...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        DrawnIndex = 0
        for value in values:
            prob = value/totalValue
            if rand - prob < 0:
                return all[DrawnIndex][0]
            DrawnIndex += 1
            rand = rand - prob
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for position in self.allPositions:
            ...
            self.beliefs[position] = self.beliefs[position] * self.getObservationProb(observation, pacPos, position, jailPos)
    ...
    def elapseTime(self, gameState):
        ...
        tempDist = self.beliefs.copy()
        for pos in list(tempDist.keys()):
            tempDist[pos] = 0

        for position in self.allPositions:
            newPosDist = self.getPositionDistribution(gameState, position)
            for p in list(newPosDist.keys()):
                tempDist[p] += self.beliefs[position] * newPosDist[p]
        self.beliefs = tempDist.copy()
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for p in list(weightDistribution.keys()):
        ...
        if weightDistribution.total() == 0:
            ...
            self.particles = [weightDistribution.sample() for _ in range(int(self.numParticles))]
    ...
    def getBeliefDistribution(self):
        ...
        for p in self.particles:
            if p not in beliefDistribution:
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for p in list(weightDistribution.keys()):
            for i in range(self.numGhosts):
                    ...
                    pacmanPosition = gameState.getPacmanPosition()
                    weight = self.getObservationProb(observation[i], pacmanPosition, p[i], jailPosition)
