...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        if len(actions) > 0:
            ...
            for action in actions:
                QValueCandidates.append(self.getQValue(state, action))
            return max(QValueCandidates)
        else:
            return 0
    ...
    def computeActionFromQValues(self, state):
        ...
        if len(actions) > 0:
            ...
            for Q in Qs:
                if Q == maxQ:
                    maxActions.append(actions[Qs.index(Q)])
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        for key in features.keys():
            tempFeatures[key] = features[key] * diff * self.alpha
        self.weights = self.weights + tempFeatures
