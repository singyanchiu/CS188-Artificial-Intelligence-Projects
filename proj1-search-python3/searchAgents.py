...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        #print("Conerns Position = ", self.corners)
    ...
    def getSuccessors(self, state):
        ...
        """
        #print ("Position to expand: ", state[0])
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            hitsWall = self.walls[nextx][nexty]
            if not hitsWall:
                nextCornersStatus = state[1].copy()
                for corner in nextCornersStatus:
                    if (nextx, nexty) == corner:
                        nextCornersStatus.pop(nextCornersStatus.index(corner))
                        break
                successors.append((((nextx,nexty),nextCornersStatus), action, 1))
...
def cornersHeuristic(state, problem):
    ...
    # #find all unvisited corners and put them in a list called cornersRemaining
    # for boolean in state[1]:
    #     if boolean == True:
    #         cornersRemaining.pop(state[1].index(boolean))
    # maxFoodPairDistance = 0
    # if len(cornersRemaining) >= 2:
    #
    #     #calculate manhattan distance between each pair of food (all permutations in foodGrid) and find the maximum of them, and
    #     #note the pair with max distance
    #     for foodPair in itertools.permutations(cornersRemaining,2):
    ...
    #
    #     #get the manhattan distance between pacman and nearest food among the max distance food pair we get above
    #     #minPacmanToFoodDistance = min([mazeDistance(currentPosition, foodPosition, problem.startingGameState) for foodPosition in maxFoodPair])
    #     minPacmanToFoodDistance = min([manhattan(currentPosition, foodPosition) for foodPosition in maxFoodPair])
    #
    #     #When only one food left, just return the manhattan distance between pacman and food
    # elif len(cornersRemaining) == 1:
    #     foodPosition = cornersRemaining[0]
    #     #minPacmanToFoodDistance = mazeDistance(currentPosition, foodPosition, problem.startingGameState)
    #     minPacmanToFoodDistance = manhattan(currentPosition, foodPosition)
    # else:
    ...
    if len(cornersRemaining) > 1:
        #find all permutations of cornersRemaining meaning all possible order of visiting them one by one
        #and return the minimum total distance travelled thru all corners
        totalDistances = []
        totalDistance = 0
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        #print("\n problem = ", problem, "\n")
        #print("\nsearch.breadthFirstSearch(problem) = ", search.breadthFirstSearch(problem), "\n")
